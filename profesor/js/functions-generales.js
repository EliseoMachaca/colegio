...
function showInstrumento(){
    ...
    if(document.querySelector('#listInstrumento')!=null){

            var request=(window.XMLHttpRequest)?new XMLHttpRequest:new ActiveXObject('Microsoft.XMLHTTP');
            var url='./models/options/options-instrumento.php';
            ...
            request.onreadystatechange=function(){
                if(request.readyState==4 && request.status==200){
                    var data=JSON.parse(request.responseText);
                    data.forEach(function(valor){
                        data+='<option value="'+valor.instrumento_ev_id+'">'+valor.instrumento_nombre+'</option>';
                    });
                    document.querySelector('#listInstrumento').innerHTML=data;
                }
            }
    }
}
