...
if(!empty($_POST)){
    ...
    if(empty($_POST['listAlumno'])||empty($_POST['listAProfesor'])||empty($_POST['listPeriodo'])){
        ...
        $sql='SELECT * FROM alumno_profesor WHERE  alumno_id=? AND pa_id=? AND periodolectivo_id=? AND estadop!=0';
        ...
        $resultInsert=$query->fetch(PDO::FETCH_ASSOC);
         ...
         $sql2='SELECT * FROM alumno_profesor WHERE  alumno_id=? AND pa_id=? AND periodolectivo_id=? AND estadop!=0 AND ap_id!=?';
        ...
        if($resultInsert>0){
            $arrResponse=array('status'=>false,'msg'=>'El alumno ya tiene el grado y el profesor asignado, selecccione otro');
         }else{
            if($idalumnoprofesor==""){
                $sql_insert="INSERT INTO alumno_profesor(alumno_id,pa_id,periodolectivo_id,estadop) VALUES (?,?,?,?)";
                $query_insert=$pdo->prepare($sql_insert);
                $request=$query_insert->execute(array($alumno,$profesor,$periodo,$status));
                if($request){
                    $arrResponse=array('status'=>true,'msg'=>'Proceso de asignacion creado correctamente'); 
                }
            }
        ...
        if($resultUpdate>0){
            ...
            if($idalumnoprofesor>0){
                $sql_update="UPDATE alumno_profesor SET alumno_id=?, pa_id=?,periodolectivo_id=?,estadop=? WHERE ap_id=?";
